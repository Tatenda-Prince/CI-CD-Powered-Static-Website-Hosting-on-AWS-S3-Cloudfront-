...
1.1.Head to your GitHub account and create a new repository by clicking “New”.
...
1.2.Give the repository a name and description. Set it to “Public”, select “Add a README file”, then click “Create repository”.
...
1.3.Navigate to “Add file”, then upload your Custom Website files.
...
1.4.After choosing your files, add a commit description, then click “Commit changes”.
...
2.1.Go to S3 Create bucket and name your bucket.
...
2.2.Uncheck Block all public access and acknowledge. Keep the default settings and click Create button.
...
2.3.Navigate to Properties tab and scroll all the way to the bottom to Edit static website hosting section. Enable Static website hosting and choose Host a static website as Hosting type. Enter the Index document name and then click Save changes button.
...
3.1.Navigate to the CodePipeline service and “Create pipeline”.


3.2.Name your pipeline and select “New service role”. Leave the rest of the settings at default, then click “Next”.
...
3.3.We can now connect our to our GitHub by selecting “Connect to GitHub”. Name the connection, then “Connect to GitHub”. You’ll need to sign in to your GitHub account to authorize the connection.
...
4.1.Proceed by adding the “Repository name” and the “Branch name”. All other settings can remain at default, then click “Next”.
...
4.2.For the “Add deploy stage”, select “S3” for the deploy provider, then the region your S3 bucket was created in, then your Resume Website bucket. Ensure to select “Extract file before deploy”, then click “Next”.
...
If you head to your S3 bucket that host’s your Resume Website files, you should see the newly updated files, including the “READMe.md” file. This verifies that our CI/CD pipeline was able to make updates aligning with our GitHub repo.We can now proceed to the final Step: Verifying our CI/CD pipeline!



## Step 5: Verify functionality of CI/CD Pipeline

5.1.Before we can fully verify the functionality of the CI/CD pipeline, we first need to verify that we can view our Custom Website from our custom s3 endpoint domain in our browser.


5.2.Go to S3Your bucket and confirm if you have `README.md `and storage.html file.
...
5.3.Click Properties and scroll to the bottom and copy the Bucket website endpoint address and paste in in the web browser.
...
5.4.Now, we will make an update to the code in GitHub to verify that the pipeline is triggered and the CI/CD pipeline is functional. This will be as easy as simply making a change to the “README.md” file, since any change to the files should trigger the workflow.


5.5.We are going to clone our repository to a local environment and use Git to push changes for our code in GitHub. I am going to add a new paragraph and write something like "WE ARE THE BEST CLUB IN LONDON"
...
![images alt](https://github.com/Tatenda-Prince/Hosting-A-Static-Website-With-CI-CD-Pipeline-GitHub-And-S3-Cloudfront/blob/ce7ef4204c70528631486a3c620dc5950268eb29/Images/Screenshot%202025-02-15%20131033.png)
